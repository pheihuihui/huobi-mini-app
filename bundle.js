(() => {
  ...
  // src/client/view/SymbolBoard.tsx
  var import_react = __toModule(require_react());
  var SymbolBoard = (props) => {
    const quoteCoins = ["usdt", "husd", "btc", "eth", "ht"];
    const allCoins = Object.keys(props.board);
    return import_react.default.createElement("table", null, import_react.default.createElement("thead", null, import_react.default.createElement("tr", null, import_react.default.createElement("td", null, "as"), quoteCoins.map((x) => import_react.default.createElement("td", null, x)))), import_react.default.createElement("tbody", null, allCoins.map((x) => import_react.default.createElement("tr", null, import_react.default.createElement("td", null, x), quoteCoins.map((v) => {
      let val = props.board[x][v];
      if (val) {
        if (val.maxOrderValue) {
          return import_react.default.createElement("td", null, `[${val.minOrderValue}, ${val.maxOrderValue}]`);
        } else {
          return import_react.default.createElement("td", null, `[${val.minOrderValue}, ...	 ]`);
        }
      } else {
        return import_react.default.createElement("td", null, `none`);
      }
    })))));
  };
  var getSymbolBoard = (board) => import_react.default.createElement(SymbolBoard, {
    board
  });
  ...
  function testConnection() {
    fetch(urlPrefix + "/top1s/count", reqParas).then((x) => x.json()).then((x) => console.log(x));
  }
  function listAllTop1s() {
    fetch(urlPrefix + "/top1s", reqParas).then((x) => x.json()).then((x) => console.log(x));
  }
  function listAllSymbols() {
    fetch(urlPrefix + "/board", reqParas).then((x) => x.json()).then((x) => {
      let bd = getSymbolBoard(x.content);
      import_react_dom.default.render(bd, panel);
    });
  }
